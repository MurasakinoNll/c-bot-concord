...
static void on_role_add_hamudi(struct discord *client,
                               struct discord_response *resp) {
  u64snowflake channel_id = (u64snowflake)(intptr_t)resp->data;
  struct discord_create_message params = {.content = "fr"};
  discord_create_message(client, channel_id, &params, NULL);
}
static void on_role_add_fail(struct discord *client,
                             struct discord_response *resp) {
  u64snowflake channel_id = (u64snowflake)(intptr_t)resp->data;
  char buf[256];
...
void role_member_add(struct discord *client,
  ...
  u64snowflake roleid = event->mention_roles->array[0];
  struct discord_add_guild_member_role params = {.reason = "role_member_add"};
  struct discord_ret ret = {
      .done = &on_role_add_done,
      .fail = &on_role_add_fail,
      .data = (void *)(intptr_t)event->channel_id,
  };
  discord_add_guild_member_role(client, event->guild_id, uid, roleid, &params,
                                &ret);
}

void role_member_remove(struct discord *client,
                        const struct discord_message *event) {
  if (event->author->bot)
    return;
  ...
  struct discord_remove_guild_member_role params = {.reason =
                                                        "role_member_remove"};
...
#define MOTAKHALIF2 435169117222207489
#define VERIFIED 1155156368957902958ULL
  if (event->author->bot)
    return;
  if (event->author->id == MOTAKHALIF1 || event->author->id == MOTAKHALIF2)
    return;
  struct discord_ret ret = {
  ...
  discord_add_guild_member_role(
      client, event->guild_id, 1255503080842330264, DUNGEONR,
      &(struct discord_add_guild_member_role){0}, NULL);
  discord_remove_guild_member_role(
      client, event->guild_id, 1255503080842330264, VERIFIED,
...
#define VERIFIED 1155156368957902958ULL
  if (event->author->bot)
    return;
  if (event->author->id == 1255503080842330264)
    return;
  struct discord_ret ret = {
      .done = &on_role_add_hamudi,
      .fail = &on_role_add_fail,
...
#define UNVERIFIED 1206585974956490802ULL
  ...
  discord_add_guild_member_role(client, event->guild_id, uid, COCCAT,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, COCMEMBER,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_remove_guild_member_role(
...
#define GUEST 1155152569526669385ULL
  ...
  discord_add_guild_member_role(client, event->guild_id, uid, GUEST,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, CAT1,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, CAT2,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, CAT3,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, CAT4,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_add_guild_member_role(client, event->guild_id, uid, VERIFIED,
                                &(struct discord_add_guild_member_role){0},
                                NULL);
  discord_remove_guild_member_role(
      client, event->guild_id, uid, UNVERIFIED,
