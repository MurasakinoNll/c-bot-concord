...
#define VICON_URL                                                              \
  "https://media.discordapp.net/attachments/1171017496220938352/"              \
  "1263918534426365952/"                                                       \
  "Confluence_-_Final_outlined.png?ex=6a7f57dd&is=6a7e065d&hm="                \
  "215bd1246fdf4cdd468408402e41602589034bcc9ed0b8a1befbf836d8ff23e5&format="   \
  "webp&quality=lossless&width=960&height=960&"
...
void on_interaction_create(struct discord *client,
  ...
  if (!event->data || !event->data->custom_id) {
    printf("event rejected]n");
    return;
  }
  if (event->type != DISCORD_INTERACTION_MESSAGE_COMPONENT) {
    printf("interaction component failed]n");
    return;
  }
...
void welcome_message(struct discord *client, u64snowflake targetchannel) {
  ...
  discord_embed_set_url(&embed,
                        "https://github.com/MurasakinoNll/c-bot-concord");
  discord_embed_set_footer(&embed, "github.com/MurasakinoNll/c-bot-concord",
                           ICON_URL, NULL);
