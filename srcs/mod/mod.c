...
#include <stdio.h>
#include <time.h>
static void on_modaction_success(struct discord *client,
...
static void
on_modaction_member_success(struct discord *client,
                            struct discord_response *resp,
                            const struct discord_guild_member *mem) {
  u64snowflake channel_id = (u64snowflake)(intptr_t)resp->data;
  char buf[256];
  snprintf(buf, sizeof(buf), "modaction succeeded for <@%" PRIu64 ">.",
           mem->user->id);
...
void ban(struct discord *client, const struct discord_message *event) {
  ...
  if (event->mentions && event->mentions->size > 0) {
    ...
    if (targetuid == 821998767652995083ULL)
      return;
  } else {
